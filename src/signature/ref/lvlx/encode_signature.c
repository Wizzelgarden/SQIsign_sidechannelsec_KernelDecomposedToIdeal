...
secret_key_to_bytes(byte_t *enc, const secret_key_t *sk, const public_key_t *pk)
{
    ...
    enc = ibz_to_bytes(enc, &sk->secret_ideal.norm, FP_ENCODED_BYTES, false);
    {
        ...
        quat_left_ideal_t tmp_id;
        quat_left_ideal_init(&tmp_id);
        ibz_mat_4x4_copy(&tmp_id.lattice.basis,&sk->secret_ideal.lattice.basis);
        ibz_copy(&tmp_id.lattice.denom,&sk->secret_ideal.lattice.denom);
        ibz_copy(&tmp_id.norm,&sk->secret_ideal.norm);
        tmp_id.parent_order = sk->secret_ideal.parent_order;
        quat_lattice_hnf(&tmp_id.lattice);
        int ret UNUSED = quat_lideal_generator(&gen, &tmp_id, &QUATALG_PINFTY);
        quat_left_ideal_finalize(&tmp_id);
        assert(ret);
